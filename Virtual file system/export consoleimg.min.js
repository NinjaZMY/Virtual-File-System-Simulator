...
let log,error,warn,trace,table,info=console.info;
log = console.log ; 
warn=console.warn;
error=console.error;
trace=console.trace; 
table=console.table; 
//automate console features functions, priority X+1

/* export function Data__Control(...o)
{
  let n=o.length; 
  for(let i=0;i<n;i++)
  {
    if(typeof o[i]=="undefined") {log(o[i]);return 0}
  }//end of for 
return 1; 
}// end of Data__Control Function     ;  */

/* export function  AddUser(id, lib) 
{    
    if( !Data__Control(Users) ) 
    {
         log("failed in Adding User")
         return log(false)
    }//end of Data_Control if  
...
{   
    if( !Data__Control(Users) ) return log(false); 
    let n=Users.length ; 
    if (n==1) return warn("there are no Users", Users[0])  ;
    for(let i=1;i<n; i++ ) 
    {
    console.info(Users[i]);
    }//end of for loop
} //end of ALL_Users function // */

export  let a = {  Users : [{ start_index: 1 }],Folders : [{ start_index: 1 }],
query : [{ start_index: 1 }], Loading:false, 
    loader:function()
    {
        onload = () =>  this.start() ;   
        this.Loading = true ; 
        return "Loading start function Sequence"
    },
    start:function()
    { /* let a="a";trace(this.b());  */
     if (!this.Loading) return this.loader();    
...
this.AddUser(uid, "user 0");

// at last fix the Folder x User problem 
// Folder x User is suppossed to be fixed here 

log(   "                                                                🐱‍👤🐱‍👤🐱‍👤🐱‍👤🐱‍👤🐱‍👤🐱‍👤🐱‍👤🐱‍👤\n",null  , ()=>{ "                                                  function start has already started                                     " }     ,this.Loading, "\n                                                               🤖🤖🤖🤖🤖🤖🤖🤖🤖")
return JSON.stringify(this.Users[1]);
...
//return AngularState; 
     ...
     , 
     b:function(){  let b="b"; log(b); return b}, 
     c:function(obj){ 
  ...
  ,test:function(x)
    ...
    log("", typeof x=="string" ? "String type Found, not an Object" :  x==null?
     ( typeof  x  ) : ( this.c(x)=="{}" ? "Cannot Call C Function of Empty" : this.c( x)   ) , "\n",  "x == null => ", x==null , "\n x equals \"", x , "\""
    )
        } //end of object test function
  ,AddUser:function (id, lib) {
        let len = this.Users.length;
        if (lib == "") { lib = "user" + len; }

        this.Users.push({ id: id, lib: lib })

    } // end of function AddUser 
  ,AddFolder: function (id, uid, lib, parent_id)
  {
    // var L;
...
/* export function ALL_Users()
{   if(typeof Users=="undefined") return console.log(false); 
    let n=Users.length ; 
    for(let i=1;i<=n; i++ ) 
    {
    console.info(Users[i]);
...
/* export function  AddUser(id, lib) {
    ...
    return Users.push({ id: id, lib: lib })
...
// or it could be replaced by log=console.log
...
/* let log,error,warn,info=console.info;
log = console.log ; 
warn=console.warn;
error=console.error; */
...
let Users = [{ start_index: 1 }]; // commented to match Users scope 
    //object Notation isn't needed for Data Transfer 
            //- bullshit - it is needed
// A Must have  Oriented object instatement can be build 
//using the notation below
//below cancel > comment block toggle index - 
Users.AddUser=AddUser;
Users.ALL_Users=ALL_Users; 
Users.AddFolder= AddFolder; 
// above cancel > comment block toggle index
//log( {"Users":Users,"Folders":Folders,"log":log ,"warn":warn ,"info":info  }); // ffor debug let's go 

AddUser("") //AddUser(uid, "user 0");
    // as for uid, it depends on the user session => get user-id => uid=user-id 
    
//below addFolder milestone
    
// Trying Folder oritented object patern     
// Folders.AddFolder=AddFolder; 
//for instance disabling Oriented object Folder notation 
// testing whether object Notation is essential for Data Transfer 

    //comment block toggle index below
// at last fix the Folder x User problem 
// Folder x User is suppossed to be fixed here 
let max_Level = 0;
let root_id = AddFolder(fid, uid, "", null);// this is root 
...
return {"Users":Users,"Folders":Folders,"log":log ,"warn":warn ,"info":info, "error":error,"trace":trace,"table":table  }
//automate return object above, priority X+0
//return should be replaced by eval("Object({5:5})")
